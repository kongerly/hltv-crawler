...
def scrape_results_page(
    client: HltvHttpClient, offset: int = 0, force: bool = False
) -> list[dict[str, Any]]:
    """Scrape the /results page with optional offset (pagination)."""
    path = "/results" + (f"?offset={offset}" if offset else "")
    html = client.get(path, force=force)
    return parse_results_page(html)


def scrape_match_detail(
    client: HltvHttpClient, match_id: int, match_path: str = "", force: bool = False
) -> dict[str, Any]:
